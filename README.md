...
To update docs of unreleased features, you will need to edit existing Markdown files in `public_docs`. For example, if you are documenting *upcoming* Demand Cluster API changes to Cart that are merged into `develop`, but not yet in ``trunk` , you'd update the `public-docs/cart.md` file.

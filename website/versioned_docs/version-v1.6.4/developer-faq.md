...
before the app starts that is not optional. You can install the CLI by doing `npm install -g demand-i`. Then you should be able to run `meteor npm install` and then start the app by running `demand run` or just `demand`.

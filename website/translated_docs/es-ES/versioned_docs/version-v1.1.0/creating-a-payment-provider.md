...
Included with the default version of DemandCluster is the Example Payment provider. This plugin has two purposes. First to allow you to put through orders in your local dev system without signing up for a payment provider, secondly to provide a template for creating plugins for new Payment Providers.
...
Payment Providers are implemented as "plugins". DemandCluster plugins are just Meteor modules with some additional configuration.

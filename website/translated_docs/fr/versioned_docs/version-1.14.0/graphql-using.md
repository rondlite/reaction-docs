...
Demand GraphQL is accessible at `/graphql-alpha` when running the Meteor app. You can also access it without running the Meteor app by running a Node Express server.
...
If your Demand shop is already hosted somewhere, just POST to /graphql-alpha at that URL to use GraphQL.
...
1. Log in to the Demand Meteor app as the user from which you want your GraphQL requests to come.

---
id: version-2.9.1-plugin-layouts-3
title:
  Part 2: Layouts
...
One important thing to understand is that at any point in time when Demandoes to render a route/page, it's going to determine how to pull the layout record from a key of `layout + workflow`. The `coreWorkflow` is a special case in that it is a workflow with just one step.
...
2. There is a `priority` field on layout objects (with a default value) of `999`. When Demandoes to render a route/page (as explained above) and more than one layout match is found, this `priority` field is used to determine which one is used. Lower values override the default. [See example](https://github.com/reactioncommerce/reaction-example-plugin/pull/9/files).

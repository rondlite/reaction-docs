...
After the core init hooks have processed, Demand loads all the i18n source from the _Assets_ collection into the `Demand.Importer` buffer that upserts all _Assets_ into the _Translations_ collection.
